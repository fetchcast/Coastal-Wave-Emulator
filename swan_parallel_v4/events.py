...
def parse_jma(text):
    ...
    while i < len(lines):
        ...
        i += count + 1
    if not storms:
        raise ValueError('No 2021 storms found in JMA source')
    return storms
...
def build_events(nc, source_dir, protocol, output):
    ...
    for storm in storms:
        ...
        start = max(int(selected[0])-padding, int(np.datetime64('2021-01-01T12','h').astype('int64')))
        end = min(int(selected[-1])+padding, int(np.datetime64('2021-12-31T23','h').astype('int64')))
        if start > end:
            continue
