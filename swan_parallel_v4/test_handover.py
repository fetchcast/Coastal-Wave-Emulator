...
class Handover(unittest.TestCase):
    def test_only_owned_tree_stops(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            parent=subprocess.Popen([sys.executable,'run_campaign.py'],cwd=pkg)
            ...
            try:
                ...
                stop_old(server,server/'runs/new')
                waiter.join(timeout=5)
                self.assertEqual(parent.returncode,0);self.assertIsNone(unrelated.poll())
                self.assertTrue((server/'runs/new/handover.json').exists())
            finally:
