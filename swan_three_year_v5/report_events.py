...
def event_plot(root, event, series):
    ...
    models=sorted({m for m,s in series})
    colors={m:plt.get_cmap('tab10')(i%10) for i,m in enumerate(models)}
    for model in models:
...
def spatial_plot(root, event, target_index):
    ...
    import matplotlib.pyplot as plt
    arrays={}
    models=sorted({e['job']['model'] for e in read(root/'selected_2022.json')})
    for model in models:
    ...
    for var,ch,unit in [('hs',0,'m'),('tm',1,'s'),('dir',None,'degrees')]:
        ...
        fig,axs=plt.subplots(len(models),3,figsize=(12,3.7*len(models)),layout='constrained',squeeze=False)
        ...
        fig.suptitle(f'{event["id"]} {event["name"]}: {var.upper()}, seed 42, {arrays[models[0]]["time"]} UTC\nShared full-range scales; no outlier removal')
        fig.savefig(root/'figures'/f'event_{event["id"]}_{var}_seed42.png',dpi=130);plt.close(fig)


def report(root):
    root=Path(root)
    events=read(root/'events_2022.json')['events']
    selected=read(root/'selected_2022.json')
    ...
    for entry in selected:
        ...
        times=np.array([r['time'] for r in rows],dtype='datetime64[ns]')
        expected=np.arange(np.datetime64('2022-01-01T12','h'),np.datetime64('2023-01-01T00','h')).astype('datetime64[ns]')
