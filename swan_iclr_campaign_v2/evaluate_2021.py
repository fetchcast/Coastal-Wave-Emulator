...
def year_indices(values,length):
    ...
    ids=np.flatnonzero((values>=np.datetime64('2021-01-01'))&(values<np.datetime64('2022-01-01')))
    if len(ids)!=8760 or not np.array_equal(values[ids],np.arange(np.datetime64('2021-01-01T00','h'),np.datetime64('2022-01-01T00','h')).astype('datetime64[ns]')):
        raise ValueError('Expected complete calendar year 2021')
...
def prepare(a):
    ...
    with xr.open_dataset(a.nc,cache=False) as ds,xr.open_dataset(a.reference,cache=False) as ref:
        ...
        info=dict(shapes={'inputs.npy':[T,10,H,W],'targets.npy':[T,4,H,W],'mask.npy':[H,W],'times.npy':[T]},
            ...
            year='2021',excluded_source_frames=int(ds.sizes['time']-T),normalization_source=entry['normalization'],
...
def frame_metrics(pred,target,mask,norm):
    ...
    for ch,name in ((0,'hs'),(1,'tm')):
        ...
        if name=='hs':output['true_hs_max']=float(tt.max());output['true_hs_mean']=float(tt.mean())
...
def evaluate(a):
    ...
    if path.exists():
        with path.open() as f:
            for row in csv.DictReader(f):
                try:
                    idx=int(row['target_index']);vals={k:float(v) for k,v in row.items() if k not in ('target_index','time')}
    ...
    atomic(a.result,dict(complete=True,signature=signature,model=job['model'],seed=job['seed'],config_id=job['config_id'],
        ...
        scope='Frozen 2019-2020 model; native SWAN direction convention; uniform kcs>0 mask; no new calibration',
        notes=['Mean frame Hs RMSE matches the aggregation used by legacy rmse_m.',
               ...
               'First 12 hours excluded for input context; no 2020 carry-in; 2022 endpoint excluded.',
