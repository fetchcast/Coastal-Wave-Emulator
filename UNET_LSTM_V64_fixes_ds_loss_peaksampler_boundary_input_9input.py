...
def wrapper(data_path, use_bnd="on"):  # "on" | "off" | "auto"
    ...
    def _auto_align_bnd_dir(bnd_feat, ds_sim, kcs2d, time_index):
        ...
        def _transform(deg, reflect):
            """(sin, cos) of the transformed BND direction.
            rotation  : theta' = theta + deg
            reflection: theta' = deg - theta   (Cartesian <-> nautical uses deg = 270)
            """
            r = np.deg2rad(deg)
            if not reflect:
        ...
        # Readable score table. Reflections are returned as 1000 + deg so the
        # caller can still treat the first return value as a number.
        scores = {("refl%+.0f" % d if rf else "rot%+.0f" % d): s
                  for (d, rf), s in scores.items()}
        best_deg = (1000.0 + best_deg) if best_reflect else float(best_deg)
